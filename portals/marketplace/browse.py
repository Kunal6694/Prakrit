...
# We use session_state to ensure the database connection persists
...
if st.session_state.view == "grid":
    ...
    # This reads the streaming pulse generated by main.py in real-time
    ...
    try:
        if os.path.exists("data/live_pulse.jsonl"):
            with open("data/live_pulse.jsonl", "r") as f:
                # Retrieve the latest state from the Pathway streaming sink
                lines = f.readlines()
                if lines:
    ...
    with tab_main:
        ...
        # Provides insights based on the live sustainability knowledge base
        with st.container(border=True):
            ...
            if os.path.exists("data/pathway_results.csv"):
                ...
                if not df.empty:
                    # Display the latest insight generated by the Pathway stream
            ...
            if st.button("Consult AI Index"):
                if user_q:
                    ...
                    # In a full RAG implementation, this would query the Pathway index
                    st.info(
                        "Based on current best practices in the Prakrit stream, prioritizing solar lighting and local material sourcing reduces event carbon impact by up to 40%.")
